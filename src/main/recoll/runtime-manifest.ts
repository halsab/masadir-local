...
const isStringRecord = (
  ...
  Object.entries(value).every(
    ([key, entry]) =>
      isSafeRelativePath(key) && typeof entry === 'string' && validate(entry),
  );
...
const isRuntimeLicense = (entry: unknown): entry is RuntimeLicense => {
  ...
  return (
    ...
    license.version.trim().length > 0 &&
    typeof license.source === 'string' &&
    license.source.trim().length > 0 &&
...
export const parseRuntimeManifest = (value: unknown): RuntimeManifest => {
  ...
  if (
    ...
    (manifest.platform !== undefined &&
      !['darwin', 'win32'].includes(manifest.platform as string)) ||
    (manifest.arch !== undefined &&
      !['arm64', 'x64'].includes(manifest.arch as string)) ||
    ...
    (manifest.checksums !== undefined &&
      !isStringRecord(manifest.checksums, (entry) =>
  ...
  return {
    ...
    ...(manifest.platform === undefined
      ? {}
      : { platform: manifest.platform as 'darwin' | 'win32' }),
    ...(manifest.arch === undefined
      ? {}
      : { arch: manifest.arch as 'arm64' | 'x64' }),
    ...
    ...(manifest.helperVersions === undefined
      ...
      : {
          helperVersions: {
            ...(manifest.helperVersions as Record<string, string>),
          },
        }),
    ...(manifest.checksums === undefined
      ? {}
    ...
    ...(manifest.licenses === undefined
      ? {}
      : { licenses: [...(manifest.licenses as RuntimeLicense[])] }),
  };
};
